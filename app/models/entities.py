...
class User(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "global_role IN ('player', 'admin')", name="check_user_global_role"
        ),
    )
...
class Tournament(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(30), nullable=False, default="registration_open", index=True
    )
    ...
    registration_closed_at: Mapped[datetime | None] = mapped_column(
...
class Team(Base):
    ...
    captain: Mapped[User] = relationship(
        "User", back_populates="captained_teams", foreign_keys=[captain_id]
    )
...
class TeamMember(Base):
    ...
    __table_args__ = (
        # Un joueur est dans au plus une équipe par tournoi
...
class Match(Base):
    ...
    team1_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("teams.id", ondelete="SET NULL"), nullable=True
    )
    team2_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("teams.id", ondelete="SET NULL"), nullable=True
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "round",
            "match_number",
            name="uq_match_tournament_round_number",
        ),
        CheckConstraint(
            "status IN ('pending', 'in_progress', 'completed', 'forfeit')",
    ...
    next_match: Mapped[Match | None] = relationship(
        "Match", remote_side=[id], foreign_keys=[next_match_id]
    )
